...
dc.printers = {};
dc.printers.filter = function (filter, resolution) {
    var s = "";

    if (filter) {
        if (filter instanceof Array) {
            if (filter.length >= 2)
                s = "[" + printSingleValue(filter[0], resolution) + " -> " + printSingleValue(filter[1], resolution) + "]";
            else if (filter.length >= 1)
                s = printSingleValue(filter[0], resolution);
        } else {
            s = printSingleValue(filter, resolution)
        }
    }
...
function printSingleValue(filter, resolution) {
    var s = "" + filter;
    ...
    if (filter instanceof Date)
        ...
        s = Math.round(filter * resolution) / resolution;
...
dc.baseChart = function (_chart) {
    ...
    _chart.roundingResoultion = function(_) {
        if (!arguments.length) return _roundingResolution;
        _roundingResolution = _;
        return _chart;
    }
    ...
    _chart.turnOnControls = function () {
        ...
        _chart.selectAll(".filter").text(_filterPrinter(_chart.filter(), _chart.roundingResoultion())).style("display", null);
        return _chart;
    };
...
dc.coordinateGridChart = function (_chart) {
    ...
    _chart.prepareXAxis = function(g) {
...
dc.singleSelectionChart = function(_chart) {
    ...
    _chart.clickEnabled = function(_){
        if(!arguments.length) return _clickEnabled;
        _clickEnabled = _;
        return _chart;
    };
...
dc.stackableChart = function (_chart) {
    ...
    _chart.groupBars = function(_) {
        if (!arguments.length) return _groupBars;
        _groupBars = _;
        return _chart;
    }
...
dc.barChart = function (parent, chartGroup) {
    ...
    function getNumberOfBars() {
        ...
        if(_chart.groupBars()) {
            _numberOfBars = _numberOfBars * _chart.allGroups().length;
        }
    ...
    function barX(bar, data, groupIndex, dataIndex) {
        ...
        var add = 0;
        if(_chart.groupBars()) {
            add = (groupIndex*(barWidth(data) + 1)); //1 for _gap hardcoded
        }
        var position = _chart.x()(_chart.keyAccessor()(data))+ add + _chart.margins().left;
        if (_centerBar)
...
dc.dataTable = function(parent, chartGroup) {
    ...
    function renderRows(groups) {
        ...
        for(var trAttr in _trAttrs) {
            var fun = _trAttrs[trAttr];
            rowEnter
                .attr(trAttr, function(d) {
                    return fun(d);
                });
        }
    ...
    _chart.trAttrs = function(_) {
        if (!arguments.length) return _trAttrs;
        _trAttrs = _;
        return _chart;
    }
...
dc.compositeChartDoubleY = function(parent, chartGroup) {
    var SUB_CHART_CLASS = "sub";
    ...
    dc.override(_chart, "renderYAxis", function(g) {
        var minus = 0;
        if(typeof _chart.x().domain()[0] === 'string')
            minus = 20;
        var axisYG = g.selectAll("g.y");
        var axisY2G;
        if (axisYG.empty()) {
            axisYG = g.append("g")
                .attr("class", "axis axisLeft y")
                .attr("transform", "translate(" + _chart.yAxisX() + "," + _chart.margins().top + ")");
            axisY2G = g.append("g")
                .attr("class", "axis axisRight y")
                .attr("transform", "translate(" + (_chart.width() - _chart.margins().right - minus) + "," + _chart.margins().top + ")");
        }
        dc.transition(axisYG, _chart.transitionDuration())
            .call( _chart.yAxis().orient("left"));
        dc.transition(axisY2G, _chart.transitionDuration())
            .call( _children[1].yAxis().orient("right"));
    });
    ...
    _chart.plotData = function() {
        for (var i = 0; i < _children.length; ++i) {
            ...
            if(i%2==0) {
                ...
                child.prepareYAxis(child.g());
                child.yAxis(_children[1].yAxis());
            }
            child.plotData();
...
/*    _chart.y2AxisMin = function() {
        return _children[1].yAxisMin();
    };

    _chart.y2AxisMax = function() {
        return _children[1].yAxisMax();
    };*/
