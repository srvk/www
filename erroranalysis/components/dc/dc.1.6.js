...
dc.utils.GroupStack = function () {
    ...
    this.betterToLayers = function () {
        var allxs = [];
        var tempDataLayers = [];
        for (var i = 0; i < _dataLayers.length; ++i) {
            var thisxs = _.map(_dataLayers[i], function(object) { 
                return object.x;
            });
            allxs = _.union(allxs, thisxs);
        }

        for (var i = 0; i < _dataLayers.length; ++i) {
            var tempDataLayer = _.clone(_dataLayers[i]);
            var newDataLayer = _.map(allxs, function(x) {
                var newObject = new Object();
                newObject.data = new Object();
                newObject.data.x = x;
                newObject.x = x;
                var y = 0;
                var find = _.findWhere(tempDataLayer, {"x": x});
            ...
            tempDataLayers.push(newDataLayer);
        }

        var layers = [];

        for (var i = 0; i < tempDataLayers.length; ++i) {
            var layer = {index: i, points: []};
            var dataPoints = tempDataLayers[i];
...
dc.stackableChart = function (_chart) {
    ...
    var _stackLayers;
    var _multiStackBetterToLayer = false;
    ...
    **/
    ...
    _chart.multiStackBetterToLayer = function (flag) {
        if (!arguments.length) return _multiStackBetterToLayer;
        _multiStackBetterToLayer = flag;
        return _chart;
    };

        var _multiStackBetterToLayer;
    ...
    _chart.stackLayers = function (_) {
        if (!arguments.length) {
            if (_stackLayers === null) {
                ...
                if(_multiStackBetterToLayer) {
                    _stackLayers = _chart.stackLayout()(_groupStack.betterToLayers());
                } else {
...
dc.barChart = function (parent, chartGroup) {
    ...
    function getNumberOfBars() {
        if (_numberOfBars === undefined) {
            ...
            if(_groupBars)
                _numberOfBars *= _chart.stackLayers().length;
        }
